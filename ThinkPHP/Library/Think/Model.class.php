...
>>>>>>> 40757e0d881b6b0d949501aa58148e1a8d77e340
        ...
        // 获取数据库操作对象
        // 当前模型有独立的数据库连接信息
        $this->tmp_config=$connection;//缓存的时候用，曾
    ...
    public function flush() {
        ...
        $this->fields   =   array_map("strtolower",array_keys($fields));//字段名都转成小写
...
>>>>>>> 40757e0d881b6b0d949501aa58148e1a8d77e340
        ...
        if(isset($options['cache'])){
            $cache  =   $options['cache'];
            $key    =   is_string($cache['key'])?$cache['key']:md5(serialize($options).serialize($this->tmp_config));
            $data   =   S($key,'',$cache);
            if(false !== $data){
                return $data;
            }
...
>>>>>>> 40757e0d881b6b0d949501aa58148e1a8d77e340
    ...
    public function getField($field,$sepa=null) {
        ...
        if(isset($options['cache'])){
            $cache  =   $options['cache'];
            $key    =   is_string($cache['key'])?$cache['key']:md5($sepa.serialize($options).serialize($this->tmp_config));
            $data   =   S($key,'',$cache);
            if(false !== $data){
                return $data;
            }
...
>>>>>>> 40757e0d881b6b0d949501aa58148e1a8d77e340
    ...
    public function getDbFields(){
        if(isset($this->options['table'])) {// 动态指定表名
            ...
            return  $fields ? array_map("strtolower",array_keys($fields)) : false;//都转成小写 2019.4.4
        }
        if($this->fields) {
            $fields     =  $this->fields;
    ...
    /**
     * 把where add addAll save 中的字段字改成小写，获取的字段字已经改成小写了
     */
    public function arr2lower($arr){
