    ...
    public function draw($code){
      // 图片宽(px)
        ...
        if ($this->useCurve) {
         ...
         $codeNX=0;
         for ($i = 0; $i<$len; $i++) {
                $codeNX  += mt_rand($this->fontSize*1.4, $this->fontSize*1.8);
                imagettftext($this->_image, $this->fontSize*1.2, mt_rand(-40, 40), $codeNX, $this->fontSize*1.6, $this->_color, $this->fontttf, $code[$i]);
            }
        // 输出图像
